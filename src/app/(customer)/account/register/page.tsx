...
import { UserPlus, ShieldCheck, CheckCircle2, Lock, Sparkles } from 'lucide-react';
        ...
        <div className="bg-white p-8 sm:p-10 rounded-3xl border border-[#EADBC8] shadow-xl space-y-6 text-center animate-fade-in">
          ...
          <div className="space-y-3">
            ...
            <h1 className="font-serif text-2xl sm:text-3xl font-extrabold text-[#2B1B1B] tracking-tight">
              {language === 'EN' ? 'Create Account with Google' : language === 'MS' ? 'Daftar Akaun Dengan Google' : 'Daftar Akun dengan Google'}
            </h1>
            
            <p className="text-stone-600 text-xs leading-relaxed max-w-xs mx-auto font-medium">
              {language === 'EN'
                ? 'Register instantly with one-click using your official Google Account.'
                : language === 'MS'
                ? 'Daftar akaun pelanggan secara instan menggunakan Akaun Google anda.'
                : 'Daftar akun pelanggan secara instan menggunakan Akun Google Anda.'}
            </p>
          </div>
          ...
          <div className="p-4 bg-[#FFF8F0] rounded-2xl border border-[#EADBC8] shadow-inner space-y-3">
            <span className="text-[11px] font-bold text-stone-700 block uppercase tracking-wider">
              {language === 'EN' ? 'Click below to register:' : 'Klik tombol di bawah untuk mendaftar:'}
            </span>
          ...
          <div className="space-y-2 pt-2 text-left text-xs text-stone-600 border-t border-stone-100">
            <div className="flex items-center gap-2 text-[11px] text-stone-700">
              ...
              <span>{language === 'EN' ? 'Instant profile activation without forms or password' : 'Aktivasi akun instan tanpa perlu isi formulir manual'}</span>
            </div>
            <div className="flex items-center gap-2 text-[11px] text-stone-700">
            ...
            <div className="flex items-center gap-2 text-[11px] text-stone-700">
              ...
              <span>{language === 'EN' ? 'Guest checkout is always supported without registration' : 'Checkout tamu tetap selalu didukung tanpa pendaftaran'}</span>
            </div>
          </div>
