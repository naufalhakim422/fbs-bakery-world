...
import { ShoppingBag, Trash2, Plus, Minus, ArrowRight, ArrowLeft, MessageCircle, ShieldCheck, ChevronRight } from 'lucide-react';
        ...
        <div className="pb-6 mb-6 border-b border-[#EADBC8] flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div className="space-y-2">
            ...
            <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-stone-500 font-medium mb-1">
              ...
              <span className="text-[#800020] font-bold">{t.cart.title}</span>
            </nav>
          ...
          {cart.length > 0 && (
            <button
              onClick={clearCart}
              className="text-xs font-bold text-stone-500 hover:text-red-600 underline transition-colors cursor-pointer self-start sm:self-auto"
            >
              {t.cart.clearCart}
            </button>
          )}
        ...
        {cart.length === 0 ? (
          ...
          <div className="bg-white rounded-2xl p-10 sm:p-14 text-center border border-stone-200 shadow-xs my-6 space-y-3 max-w-md mx-auto">
            ...
            <div className="space-y-1">
              <h2 className="font-serif text-xl font-bold text-[#2B1B1B]">{t.cart.emptyTitle}</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {/* Free Shipping Meter */}
              <div className="bg-stone-100 text-stone-800 p-4 rounded-xl border border-stone-200 shadow-xs">
                {amountToFreeShipping > 0 ? (
                  <p className="text-xs font-medium">{(t.cart?.freeShippingAdd || '').replace('{amount}', formatMYR(amountToFreeShipping))}</p>
                ) : (
                  <p className="text-xs text-[#800020] font-bold">{t.cart.freeShippingUnlocked}</p>
                )}
              ...
              {cart.map((item) => (
                <div 
                  ...
                  className="bg-white p-4 sm:p-5 rounded-2xl border border-stone-200 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4"
                >
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    />
                    <div>
                      <h3 className="font-bold text-sm sm:text-base text-[#2B1B1B]">{item.productName}</h3>
                      <span className="inline-block px-2 py-0.5 bg-[#800020]/10 text-[#800020] text-xs font-bold rounded-md mt-1">
                        {t.cart.variant}: {item.variantName}
                      </span>
                      <span className="block text-[11px] text-stone-400 font-mono mt-0.5">{t.cart.sku}: {item.sku}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto pt-3 sm:pt-0 border-t sm:border-t-0 border-stone-100">
                    ...
                    <div className="text-right">
                      <span className="block text-[10px] uppercase tracking-wider text-stone-400 font-bold">{t.cart.subtotal}</span>
                      <span className="font-serif text-base sm:text-lg font-bold text-[#800020]">
                    ...
                    <button 
                      ...
                      className="p-2 text-stone-400 hover:text-red-600 rounded-lg transition-colors cursor-pointer"
            ...
            <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-xs h-fit space-y-5">
              ...
              <div className="space-y-2.5 text-xs text-stone-600 font-medium">
                ...
                <div className="pt-3 border-t border-stone-200 flex justify-between items-baseline">
                  ...
                  <span className="font-serif text-2xl font-bold text-[#800020]">{formatMYR(subtotal)}</span>
                </div>
              </div>
              ...
              <Link
                href="/checkout"
                className="w-full py-3.5 bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-md flex items-center justify-center gap-2 active:scale-95 cursor-pointer"
              >
                <MessageCircle className="w-4 h-4 fill-white" /> {t.cart.proceedCheckout} <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
