    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredVideos.length === 0 ? (
          <div className="col-span-full bg-white rounded-3xl p-12 text-center border border-stone-200 space-y-3">
            <Film className="w-12 h-12 text-stone-300 mx-auto" />
            <h3 className="font-serif text-lg font-bold text-stone-800">{labels.noVideos}</h3>
          </div>
        ) : (
          filteredVideos.map(video => (
            <div key={video.id} className="bg-white rounded-3xl border border-stone-200 overflow-hidden shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
              
              {/* Media Thumbnail */}
              <div className="relative aspect-video bg-black group border-b border-stone-100">
                <img src={video.thumbnail} alt={video.title} className="w-full h-full object-cover group-hover:scale-102 transition-transform duration-300 animate-fade-in" />
              ...
              <div className="p-5 flex-1 space-y-2">
                <div className="flex items-center justify-between text-[10px] font-extrabold tracking-wider text-[#800020] uppercase">
                  <span>{video.category}</span>
                  <span className="px-2 py-0.5 rounded-md bg-stone-100 text-stone-600 border border-stone-200">{video.platform}</span>
                </div>
                <h3 className="font-serif font-bold text-base text-stone-900 line-clamp-2 leading-snug">{video.title}</h3>
                <p className="text-stone-500 text-xs line-clamp-3 leading-relaxed">{video.description}</p>
                <div className="text-[10px] text-stone-400 font-semibold pt-1">
              ...
              <div className="px-5 py-4 border-t border-stone-100 bg-stone-50/50 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 bg-stone-100 hover:bg-stone-200 text-stone-800 rounded-xl text-xs font-bold transition-all"
                    title="Edit"
                  >
                  ...
                  <button
                    ...
                    className="p-2 bg-red-50 text-red-700 hover:bg-red-100 rounded-xl text-xs font-bold transition-all"
                    title="Hapus"
                  >
