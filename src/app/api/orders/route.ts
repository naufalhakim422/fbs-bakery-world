...
const defaultInitialOrders: any[] = [];
...
function readServerOrders(): any[] {
  ...
  const filtered = baseOrders.filter((o: any) => !deletedIds.includes(o.id) && !deletedIds.includes(o.orderNumber) && !dummyIds.includes(o.id) && !dummyIds.includes(o.orderNumber));
...
export async function GET(request: Request) {
  ...
  try {
    // Purge initial dummy orders & customers from Railway PostgreSQL DB
    try {
      await prisma.$executeRawUnsafe(`UPDATE "Order" SET "deletedAt" = NOW(), "status" = 'Cancelled' WHERE "id" IN ('ord-1', 'ord-2', 'ord-3') OR "orderNumber" LIKE '%20260805-10%' OR "orderNumber" LIKE 'FB2600000%';`);
      await prisma.$executeRawUnsafe(`UPDATE "Customer" SET "deletedAt" = NOW() WHERE "id" IN ('cust-1', 'cust-2', 'cust-3') OR "email" = 'siti@example.com';`);
    } catch (e) {}
