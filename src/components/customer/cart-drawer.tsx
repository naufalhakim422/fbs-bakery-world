        ...
        <div className="w-screen max-w-md bg-[#FFF8F0] shadow-2xl flex flex-col border-l border-[#D4AF37]/30">
          ...
          <div className="p-5 bg-[#800020] text-[#FFF8F0] flex items-center justify-between shadow-md">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold font-serif tracking-wide">Shopping Cart ({totalItems})</h2>
            </div>
            <button 
              onClick={onClose} 
          ...
          <div className="bg-[#5A0015] px-5 py-3 text-xs text-[#FFF8F0] border-b border-[#D4AF37]/20">
            {amountToFreeShipping > 0 ? (
              <p>Add <span className="font-bold text-[#D4AF37]">{formatMYR(amountToFreeShipping)}</span> more for <span className="font-bold text-[#D4AF37]">FREE SHIPPING</span>!</p>
            ) : (
              <p className="text-[#D4AF37] font-semibold flex items-center gap-1">
                🎉 Congratulations! You unlocked Free Delivery across Malaysia!
              </p>
            )}
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-[#2B1B1B]/70 py-12">
                ...
                <h3 className="text-lg font-bold font-serif text-[#800020]">Your cart is empty</h3>
                <p className="text-sm mt-1 max-w-xs text-stone-600">Discover premium flours, chocolates, yeast & baking tools for your creations.</p>
                <button 
                  onClick={onClose}
          ...
          {cart.length > 0 && (
            <div className="p-5 bg-white border-t border-[#EADBC8] shadow-lg space-y-3">
              <div className="flex justify-between items-center text-sm">
                <span className="text-stone-600">Subtotal Estimated</span>
                <span className="text-lg font-bold text-[#800020]">{formatMYR(subtotal)}</span>
              </div>
              <p className="text-[11px] text-stone-500">Shipping costs & delivery times will be confirmed directly by admin on WhatsApp.</p>
