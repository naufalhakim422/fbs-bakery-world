      ...
      <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl border border-[#EADBC8] overflow-hidden print:shadow-none print:border-none print:rounded-none print:max-w-none">
        ...
        <div className="bg-[#800020] text-white px-6 py-4 flex items-center justify-between print:hidden">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif font-bold text-base">Salinan Invoice Resmi</h3>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              aria-label="Tutup Modal Invoice"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="p-8 sm:p-10 space-y-8 print:p-6 print:space-y-6 text-stone-800">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-stone-200 gap-4">
            <div>
              ...
              <p className="text-xs text-stone-500 mt-1">Pembekal Bahan & Hiasan Kek Terpercaya Malaysia</p>
              <p className="text-[11px] text-stone-500">Chukai, Terengganu | Hubungi: +60 18-394 2147</p>
            </div>
            ...
            <div className="text-left sm:text-right bg-stone-50 p-4 rounded-2xl border border-stone-200 w-full sm:w-auto">
              ...
              <span className="text-xs text-stone-500 block mt-1">Tarikh: {orderDate}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-stone-50/80 p-5 rounded-2xl border border-stone-200 text-xs">
            <div>
              <span className="font-bold text-[#800020] uppercase tracking-wider block mb-1.5">Maklumat Pelanggan</span>
              <p className="font-bold text-stone-900 text-sm">{order.customerName}</p>
              <p className="text-stone-600 mt-0.5">Telefon: {order.customerPhone}</p>
            </div>

            <div>
              <span className="font-bold text-[#800020] uppercase tracking-wider block mb-1.5">Alamat Penghantaran</span>
            ...
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="bg-[#800020] text-white font-bold uppercase text-[11px] tracking-wider">
                  ...
                  <th className="py-3 px-4 text-right">Harga Unit</th>
                  <th className="py-3 px-4 text-right rounded-r-xl">Jumlah</th>
                </tr>
              </thead>
                ...
                {order.items.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-stone-50">
                    ...
                    <td className="py-3 px-4">
                      ...
                      <span className="text-stone-500 text-[11px]">Varian: {item.variantName}</span>
                    </td>
          ...
          <div className="pt-4 border-t border-stone-200 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
            <div className="text-xs text-stone-500 space-y-1">
              <p className="font-bold text-stone-700">Terima kasih kerana berbelanja di FBS Bakery World!</p>
              <p>Resit rasmi ini digenerasikan secara automatik oleh sistem e-dagang FBS Bakery World.</p>
            </div>
